...
def load_project_env(filename='.env'):
    ...
    """
    # Soporta tanto .env del servicio como el .env local histórico del proyecto.
    for env_path in (Path(__file__).with_name(filename), *Path(__file__).parents[1:3]):
        env_path = env_path if env_path.name == filename else env_path / filename
...
files = glob.glob(str(Path(__file__).parent / "data" / "ES" / "matches_*.csv"))
if not files:
...
_global_console_model = None


def predict_global_console():
    """Consola visual para el modelo global sin perder el flujo histórico."""
    global _global_console_model
    from global_match_predictor import GlobalPredictor, LEAGUES, rank_betano_markets
    from betano_client import markets as get_betano_markets

    print("\n" + "=" * 100)
    print("🌍 PREDICTOR GLOBAL PROFESIONAL — ANÁLISIS POR LIGA")
    print("=" * 100)
    print("Ligas: " + " | ".join(f"{code} ({config['name']})" for code, config in LEAGUES.items()))
    league = input("🏆 Código de liga: ").strip()
    home = input("🏠 Equipo local: ").strip()
    away = input("✈️  Equipo visitante: ").strip()
    if not league or not home or not away:
        print("❌ Liga y equipos son obligatorios.")
        return

    try:
        if _global_console_model is None:
            _global_console_model = GlobalPredictor()
        prediction = _global_console_model.predict(league, home, away)
    except Exception as error:
        print(f"❌ No fue posible generar la predicción: {error}")
        return

    probabilities = prediction["probabilities"]
    expected = prediction["expected"]
    print("\n" + "=" * 100)
    print(f"⚽ {home.upper()} vs {away.upper()} — {LEAGUES[prediction['league']]['name'].upper()}")
    print("=" * 100)
    print("\n🎲 PROBABILIDADES CALIBRADAS")
    print("─" * 100)
    print(f"🏠 Local: {probabilities['home_win'] * 100:5.1f}%  |  🤝 Empate: {probabilities['draw'] * 100:5.1f}%  |  ✈️ Visitante: {probabilities['away_win'] * 100:5.1f}%")
    print(f"⚽ Goles esperados: {expected['goals']:.2f}  |  Ambos marcan: {probabilities['btts_yes'] * 100:5.1f}%")
    if "corners" in expected:
        print(f"🚩 Córners esperados: {expected['corners']:.2f}")
    if "cards" in expected:
        print(f"🟨 Tarjetas esperadas: {expected['cards']:.2f}")
    print("\n📌 Mercados con cobertura: " + ", ".join(prediction["supported_markets"]))

    market_list, error = get_betano_markets(home, away)
    if error:
        print(f"\n⚠️ Betano: {error}. Se muestran probabilidades, sin recomendación de valor.")
        return

    groups = rank_betano_markets(prediction, market_list)
    print("\n💰 VALOR CONTRA CUOTAS BETANO")
    print("─" * 100)
    labels = (("🔥 Alto valor", "high_value"), ("✅ Valor", "value"), ("🟡 Valor leve", "low_value"), ("⚪ No recomendado", "not_recommended"))
    for label, group in labels:
        picks = groups[group]
        print(f"\n{label} ({len(picks)})")
        for pick in picks[:10]:
            details = f" @ {pick['odds']:.2f}" if "odds" in pick else ""
            ev = f" | EV {pick['ev_pct']:+.1f}%" if "ev_pct" in pick else ""
            reason = f" | {pick['reason']}" if pick.get("reason") else ""
            print(f"  • {pick['market']}: {pick['selection']}{details}{ev}{reason}")
...
def main_menu():
    ...
    while True:
        ...
        print("5️⃣  Predictor global visual (todas las ligas)")
        print("6️⃣  Salir")
